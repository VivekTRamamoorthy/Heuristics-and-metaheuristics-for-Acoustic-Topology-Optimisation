function [fitness,SACAbhi,dalpha_dXi]=griddecoder_cont(genome,varargin)
...
if strcmp(skeletonmodel,'BiotHelmholtz')
    %     [fitness,SACAbhi]=AbhisolverBH(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
    %     [fitness,SACAbhi]=AbhisolverBH_fixed(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
    %     [fitness,SACAbhi]=AbhisolverBH_inline(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
    %     [fitness,SACAbhi]=AbhisolverBH_deriv(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
    %     [fitness,SACAbhi]=AbhisolverBH_deriv_temp5(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);

    
    if derivToggle==1
    [fitness,SACAbhi,dalpha_dXi]=AFSO_solver_BH_cont_adjoint(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
    
    else
        [fitness,SACAbhi]=AFSO_solver_BH_cont_adjoint(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle);
        dalpha_dXi=0;
    end
        
elseif strcmp(skeletonmodel,'EquivalentFluid')
    [fitness,SACAbhi]=AbhisolverEF(mesh,frequencies,mat,plotToggle,FEorder,displayToggle,pressureFieldToggle); % this function evaluates the problem and gives the fitness
else
